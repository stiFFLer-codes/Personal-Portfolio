...
| `/research` | Live | List of research (ADS-Cascade, CARTA, Three Voices) |
...
| `/travel` | Live | Aug 2025 Nordic/Baltic trip: survey-map plate + field log (phase 9, done) |
...
Create `src/content/places/[order]-[slug].md`. Each stop is a marker on the
`/travel` map and a row in the field log below it.

Required: `name`, `country`, `dateLabel` (generalized, e.g. "August '25"), `order`
(the travel order — also drives the route line between stops), `x` and `y` (the
marker position as a percentage of the map viewBox, 0–100), `blurb` (one honest,
human-authored line).
Optional: `anchor` (boolean, default false — the single summer-school stop, drawn
green), `photos` (array of `{ src, alt }`, `src` resolved by Astro `image()`),
`source` (`{ label, href }` — a checkable artifact under `public/`, same "verify
this" idea as the project/research sources block).

Geometry is not hand-authored: the country shapes, labels, lakes, graticule, and
the projection-correct `x`/`y` for each city are regenerated by
`scripts/build-travel-geo.py` into `src/components/travel-geo.ts`. That script
runs at authoring time only — it is **not** a runtime dependency and adds no
